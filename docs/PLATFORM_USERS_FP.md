...
**Rodadas (staff):** com rodada ativa, a ficha redireciona para `/torneios/{id}/rodadas/{n}` (pairings). Entre rodadas: primary **Iniciar próxima** / **Finalizar** no header; resumo em scoreline (`Nome  placar  ×  placar  Nome`, best-of só no meta do torneio); drop e reabrir secundários; drop exige digitar o nome do jogador no 2º passo do modal.
