...
def provision_invite_and_email(db: DbSession, user: User) -> tuple[str, InviteToken]:
    """Create invite token and send email. Returns (raw_token, row)."""
    raw, invite = create_invite(db, user)
    try:
        send_invite_email(user, raw)
    except Exception:
